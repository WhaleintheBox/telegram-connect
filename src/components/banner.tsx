...
export const Banner = () => {
  ...
  useEffect(() => {
    const adjustTickerSpeed = () => {
      ...
      if (screenWidth > 1024) {
        root.style.setProperty('--ticker-duration', '60s'); // Desktop
      } else if (screenWidth > 768) {
        root.style.setProperty('--ticker-duration', '40s'); // Tablettes
      } else {
        root.style.setProperty('--ticker-duration', '30s'); // Mobile
      }
    };
