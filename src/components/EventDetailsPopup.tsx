...
const EventDetailsPopup: React.FC<EventDetailsPopupProps> = ({ box }) => {
    ...
    const getStatusColor = (status?: string): string => {  // Modification ici pour accepter undefined
      const s = status?.toLowerCase() || '';
      if (s.includes('live')) return 'bg-green-500';
      if (s.includes('ended') || s.includes('finished')) return 'bg-red-500';
      if (s.includes('cancelled')) return 'bg-red-500 animate-pulse'; // Ajout animation pour annulé
      return 'bg-blue-500';
    };
    ...
    const SoccerDetails: React.FC = () => (
        ...
        <div className="bg-gray-800/50 p-4 rounded-lg">
            ...
            <div className="flex items-center justify-between">
                ...
                {sportData.home_score !== undefined && (
                <p className="text-2xl font-bold text-white">{sportData.home_score}</p>
                )}
            </div>
            <div className="flex items-center justify-between">
                <div>
                ...
                {sportData.away_score !== undefined && (
                <p className="text-2xl font-bold text-white">{sportData.away_score}</p>
                )}
            </div>
            </div>
        </div>
      ...
      <div className="flex justify-between items-start mb-6">
        ...
        <div className="flex items-center space-x-2">
          <span className={`${getStatusColor(sportData.status)} px-3 py-1 rounded-full text-white text-sm font-medium flex items-center`}>
            {sportData.status === 'live' && <span className="w-2 h-2 bg-red-500 rounded-full animate-pulse mr-2" />}
            {sportData.formattedStatus || sportData.status}
          </span>
        </div>
      </div>

      {/* Contenu spécifique au sport avec une transition */}
