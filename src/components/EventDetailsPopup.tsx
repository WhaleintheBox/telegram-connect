...
// Ajout d'une interface de props plus cohérente
...
/**
 * Génère un historique de paris factice pour le test (à enlever en prod)
 */
...
const EventDetailsPopup: React.FC<{ box: BoxType }> = ({ box }) => {
  ...
  // Exemple de bettingHistory généré fictivement
  const bettingHistory = mockBettingHistory(24);

  // Hook custom pour déterminer le statut (live, finished, etc.)
  const status = useEventStatus(sportData);

  /**
   * Affiche un badge de statut coloré selon le statut court
   */
  const StatusBadge = ({ status }: { status: Status }) => {
    ...
    return (
      <span
        className={`px-4 py-2 rounded-full text-sm font-medium inline-flex items-center gap-2 border ${getStatusStyle()}`}
      >
  ...
  /**
   * Composant d'affichage du score d'une équipe,
   * gérant plusieurs formats de score (simple, current/total, etc.)
   */
  const TeamScore = ({
    score
  }: {
  ...
  /**
   * Graphique Recharts pour l'historique ou la tendance des bets
   */
  const BettingChart = () => (
        ...
        <div className="mt-4 pt-4 border-t border-blue-500/20">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-blue-100 font-medium truncate">{box.sportId}</p>
            </div>
          </div>
        </div>
      </div>
